...
| **S1 Uniswap v4 FX** | FX Desk: Sepolia quote + guarded UR swap + Kenji viz + `FEEDBACK.md` | K7 | **PARTIAL → S1b** — build `1d2f558` (pool/till/quotes/desk/viz); guards **not on chain** (OOG). Teller **funded**; validate via [`KICKOFF-S1-fx-validate.md`](./KICKOFF-S1-fx-validate.md). Spec [UNISWAP.md](./UNISWAP.md) · build kickoff [`KICKOFF-S1-uniswap-fx.md`](./KICKOFF-S1-uniswap-fx.md) |

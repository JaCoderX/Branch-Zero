...
| **U4+ Priority release** | Bob waits the clock; Walker meta-bypass + Passkey before `releaseTime` | G5b | ENG-0012 + 0011 + 0013 — **met 2026-09-07** (ROLE_SET 3 split on chain, `/priority/*`, Passkey step-up in the overlay, policy pins the silent lane to `params.action`; kill tests Y0–Y9) |
...
| **S1 Uniswap v4 FX** | FX Desk: Sepolia quote + guarded UR swap + Johnny viz + `FEEDBACK.md` | K7 | **MET 2026-09-08** — build `1d2f558`, landed by S1b: guard + role config on chain and a live swap `0xd98efc64…`; K7-a…f green. Owed: the sponsor feedback form. Spec [UNISWAP.md](./UNISWAP.md) · record `docs/progress/2026-09-08-s1b-fx-validate.md` |
...
| **Sepolia Ops Treasury** | `SEPOLIA_TREASURY_PK` collects ETH+USDC; background staff top-ups at need × 1.25 | Live ops | **met** 2026-09-08 — [SEPOLIA-TREASURY.md](./SEPOLIA-TREASURY.md) §8–9; owed: faucet + `treasury:topup -- --execute` ([OWED.md](./OWED.md) §1) |
| **Load Account (Iris)** | Load a player-owned AccountBlox by number (non-latest clone / multi-account); terminal discovers, Iris loads | Account Opening | **met** 2026-09-08 — `POST /account/load` after `getCode` + `owner()` + `initialized()` + `ISecureOwnable`; policies re-pinned **before** the switch; sync without `cloneBlox`. Bridge `s2.1`; `killtests:load` **12/12** on 1337 — [LOAD-ACCOUNT.md](./LOAD-ACCOUNT.md) §8 · HANDOFF-CC §5l. Owed: a Live browser walk |

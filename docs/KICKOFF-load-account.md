---
title: Kickoff prompt — Load Account (Ines)
created: 2026-09-08
product: Branch-Zero
...
# Kickoff prompt — Load Account (Ines)

Paste into a **new** Claude Code / Cursor session. Prefer **Opus 5 high** (Claude Opus 5 · high thinking).

**What this is:** Ines (Account Opening) lets the player **load a specific AccountBlox address** they own on
the current wing — for non-latest CopyBlox clones and multi-account use. Terminal stays discovery; Ines is
the clean load path.
...
hand Ines the address the Console already helped them find.
...
MISSION: Branch Zero — let Ines load a player-owned AccountBlox by address on the current wing (edge: not the latest CopyBlox clone; multi-account). Terminal remains discovery; Ines is the clean load path in chat.
...
2. https://github.com/JaCoderX/Branch-Zero/blob/main/docs/HANDOFF-load-account.md
3. https://github.com/JaCoderX/Branch-Zero/blob/main/docs/KICKOFF-load-account.md
4. https://github.com/JaCoderX/Branch-Zero/blob/main/docs/NPCS.md (Ines §4.2)
5. https://github.com/JaCoderX/Branch-Zero/blob/main/docs/TERMINAL-CONSOLE.md
6. https://github.com/JaCoderX/Branch-Zero/blob/main/docs/HANDOFF-CC.md
...
- Live vs Dev: address must be on the current desk wing — no cross-chain paste.
...
SEQUENCE:
1. Dialogue: Ines open/done/start_over — "Load an existing account" → form (mirror name_claim / payment_slip). Hint that the desk terminal helps find addresses.
...
5. Docs: NPCS Ines lines, LOAD-ACCOUNT §7 ticks, REFLECTION row, HANDOFF-CC when met. Optional one-line terminal hint toward Ines.
...
- Optional stretch: list this owner's `BloxCloned` clones as Ines choices (still log-based).

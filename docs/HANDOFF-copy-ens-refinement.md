...
kickoff: docs/KICKOFF-copy-ens-refinement.md
status: open
...
**Kickoff (paste):** [`docs/KICKOFF-copy-ens-refinement.md`](./KICKOFF-copy-ens-refinement.md)

**Status (2026-09-10):** **Open** — review-first refinement mission. Ship cheap copy + ENS surface wins; propose (do not silently invent) larger ENS features that need new bridge/desk work.
...
1. **Review all dialogues** — improve lines that are flat, jargon-leaking on the main path, stale (Ruth→Bob, Counter 1/2), or misaligned with how the bank actually works now.
...
|------|---------|------------------|
| `greeter.json` | Mo | Routing / tutorial; check Counter wording, ENS awareness |
| `clerk.json` | Ines | Open / load / faucet / terminal; bank words on main path |
| `teller.json` | Dev | Pay / wire; pay-by-name should feel first-class |
| `vault_keeper.json` | **Bob** (not Ruth) | Wait-path release only |
| `manager.json` | Okafor | Priority + shredder; no post-clock stamp |
| `registrar.json` | Petra | Claim / tier / board — main path still says “AccountBlox” in places |
| `dealer.json` | Kenji | EUR + ILS; Sepolia-only honesty |
...
| Greeter / teller / Mo lines that route by name once claimed | Low | Copy + conditions only |
...
4. **Re-run** `run_checks` when Godot host is available; light mock walk of Mo → Ines → Petra → Counter (name on slip) → Bob/Okafor one-liners if touching those graphs.
...
- Preserve: U4 freeze, U4+ Priority, Bob wait-only, Petra Name Desk, FX Sepolia-only, Live Main / Dev 1337, partners board bank voice, exterior easter separate from interior.
...
- Internal node ids (`Counter1`, etc.) may stay; **player-facing** words must stay consistent (Counter singular, Bob not Ruth, Name Desk not Counter 2).

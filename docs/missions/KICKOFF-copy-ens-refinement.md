...
| Vault keeper is **Bob**; Counter is singular | Any Bob / Counter 1 / Counter 2 in player text is stale |
...
- Player-facing consistency: Bob (not Bob); Counter (not Counter 1/2); Name Desk for Petra; ARC floor stays "coming soon".
- Preserve U4 freeze, U4+ Priority (Walker), Bob wait-only release, FX Sepolia-only, Live Main / Eve 1337, partners board bank voice, exterior easter separate.
...
5. Verify — run_checks.gd when Godot 4.5 host available; light mock walk Ash → Iris → Petra → Counter (pay by name) → Bob/Walker lines you touched.
...
- [ ] No player-facing Bob / Counter 1 / Counter 2 / wrong vault role
...
Principal: hard-refresh `:5173` if the web pack was rebuilt; walk Ash → Petra claim (or existing name) → Counter pay-by-name → glance plaques (Name Desk, Counter, FX, partners, elevator). Confirm names feel like the bank’s pointer, not a side quest.

---
type: handoff
title: Handoff — Ash the greeter knowledge graph
audience: cold agent (Codex Luna)
created: 2026-09-10
product: Branch-Zero
mission: Expand Ash (greeter.json) from thin router into a hub-and-spoke conversation knowledge graph — bank services + optional sponsor/tech Ask-why — without stealing desk verbs or competing with the iNPC
...
# Handoff — Ash the greeter knowledge graph
...
**Baseline audit (2026-09-10):** Ash’s graph in `apps/game/dialogue/greeter.json` is ~8 nodes — booting / no_account / has_account / pending + one lore hop + one vault hop. That matches historical `docs/NPCS.md` §4.1 (“routing, tutorial pacing”) but under-serves players who want to deepen on **the bank** and **the stack** from the lobby.

**Not this mission:** Rewriting Iris / Eve / Johnny / Petra Ask-why essays · turning Ash into a write lane · iNPC prompt/pack redesign as primary work · gas paymaster / 4337 sponsorship · Arc revive · U7 ship packaging · GameLab ENG-* · protocol Solidity · DEMO-SCRIPT end-to-end rewrite · dialogue *box layout* (already met).

---

## Principal intent

Make Ash the lobby’s **curated knowledge hub**:
...
3. **Always hand off** — Ash explains and points; staff at desks still own the verbs (sign in, pay, wire, release, claim name, swap).
...
**“Sponsor” here = ETHOnline partners** (Privy · ENS · Uniswap; Arc listed / elevator “coming soon”) — **not** gas sponsorship / paymasters. If a player asks who pays the fee on a stamp, Ash may say (under Ask why) that the teller’s desk broadcasts and pays gas for routine slips; do not invent account-abstraction sponsorship.
...
| **Desk Ask-why stays authoritative** | Iris owns Privy depth; Johnny owns Uniswap depth; Petra owns ENS depth; Eve owns Lane A/B depth. Ash = **one beat + send them to the desk** |
| **Conditioned routing** | Soft-bias with existing `GameState.facts()`: `!has_account` → Iris; `pending>0` → Bob / Walker; `!has_ens_name` → Petra invite; never print bank name unless `has_ens_name` |
...
hub_tour  (or inline choices on no_account / has_account / pending)
  ...
  ├─ How do I open an account?    → Iris route (+ Ask why → Privy one-beat)
  ├─ Payments & the vault         → Counter / Vault / Manager (+ existing vault tech)
  ├─ Bank names                   → Petra (+ Ask why → ENSv2 one-beat)
  ├─ Currency exchange            → Johnny (+ Ask why → Uniswap v4 one-beat)
...
|--------|------------------|--------------------|
| Account Opening · Iris | Sign in once, consent once | Privy embedded wallet + scoped session signer + policy |
| Counter · Eve | Small payments clear now; big ones go to the vault | Meta-tx; teller = broadcaster; desk pays gas for the stamp |
| Vault · Bob | Cooling clock, then release | Timelock / `releaseTime` (reuse / extend existing nodes) |
| Manager · Walker | Priority hand-scan or shred | Meta-approve / cancel; Passkey for Priority |
| Name Desk · Petra | Claim `you.branchzero.eth` | ENSv2 subname + passbook text |
| FX Desk · Johnny | Practice USD → EUR \| ILS | Guarded Uniswap v4 on Sepolia; three whitelisted calls |
...
| Ash script today | [`docs/NPCS.md`](../NPCS.md) §4.1 — **update when graph ships** |
| Privy | [`docs/PRIVY.md`](../PRIVY.md) · Iris `clerk.json` `why_login` / `why_delegate` |
| Uniswap FX | [`docs/UNISWAP.md`](../UNISWAP.md) · Johnny `dealer.json` · FX sponsor plaque in `bank_interior.gd` |
| ENS | [`docs/ENS.md`](../ENS.md) · Petra `registrar.json` |
| Partners board | Lobby plaques (`PARTNERS · PRIVY · ENS · UNISWAP · ARC`) |
| iNPC teaching facts | `apps/web/src/inpc/pack.ts` `TEACHING_PACK` — keep Ash facts aligned if you touch both |
...
2. **`docs/NPCS.md` §4.1** — rewrite Ash’s script to match the shipped graph (cold agents read this).
3. **`apps/game/tests/run_checks.gd`** — update any greeter string pins; keep ENS unnamed quiet (`has_ens_name` discipline).
4. **Optional light sync** — if Ash’s sponsor one-beats contradict `TEACHING_PACK`, fix pack wording to match (no iNPC feature work).
...
- [x] `no_account` can reach directory + Iris route + lore without dead ends
- [x] `has_account` hub covers Counter / Vault / Name / FX / partners (directory nests the side hall)
- [x] `pending>0` still highlights Bob / Walker and can deepen on vault
...
- [x] Ash has **no** `action` / write verbs
...
- Preserve: U4 freeze, Bob wait-only, Walker Priority, FX Sepolia-only, Live Main / Eve 1337, partners board bank voice, ENS passbook polish, iNPC shell methods.

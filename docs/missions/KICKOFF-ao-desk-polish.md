---
title: Kickoff — AO desk polish (Iris terminal · keyboard · Counter labels)
created: 2026-09-08
product: Branch-Zero
model: Codex Luna
...
**Why:** Principal playtest notes — (1) Iris should point players at the desk computer she already has; (2) AO keyboard sits behind the screen from the client walk-up; (3) west column should read **Counter** + **Name Desk**, not Counter 1 / Counter 2.
...
| Fact | Implication |
|------|-------------|
| `clerk.json` has no terminal choice today | Add `open_console` on Iris’s post-account path; reuse `GameState` / bridge — no new method |
| `terminal.json` already owns observer verbs | Iris may open the Console only; do not duplicate grant/revoke on Iris |
...
1. Iris choice → same `open_console` action as the physical terminal.
2. Keyboard between client and screen at Account Opening.
3. Player-facing labels: **Counter** (Eve’s bay) and **Name Desk** (Petra’s bay). No “Counter 1” / “Counter 2” on plaques or NPC lines the player hears/reads.
...
MISSION: Branch Zero polish — (1) Iris dialogue action to open the Account Opening desk terminal, (2) move AOKeyboard between AOScreen and the client, (3) rename Counter 1 → Counter and remove Counter 2 label so signage reads Counter + Name Desk only.
...
5. https://github.com/JaCoderX/Branch-Zero/blob/main/docs/NPCS.md   (§4.2 Iris; update Counter wording if you touch player copy)
...
DoD:
- [ ] Iris (account open) offers Use the desk terminal → openConsole path; overlay close restores movement + focusCanvas
- [ ] From AO customer chairs: keyboard visibly between player and screen; screen still faces client
- [ ] West column plaques: COUNTER at Eve's bay; no COUNTER 2; Name Desk identity clear for Petra

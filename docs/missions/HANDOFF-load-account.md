---
type: handoff
title: Handoff — Load Account (Iris custom AccountBlox)
...
mission: Iris loads a player-owned AccountBlox by address (non-latest clone / multi-account)
...
# Handoff — Load Account (Iris)

> **MET 2026-09-08.** Iris offers *Load an existing account* from `open`, `done` and `start_over`; the desk
...
> Sepolia; the desk paths are headless-proven on Eve. The optional stretch (listing this owner's clones as
> Iris choices) was **not** built.
...
**Authorized construction:** Iris can **load a custom AccountBlox address** the player owns on the current
wing. Prefer **Claude Code · Opus 5 high**.

**Plan:** [`docs/LOAD-ACCOUNT.md`](../LOAD-ACCOUNT.md)  
**Kickoff (paste):** [`docs/missions/KICKOFF-load-account.md`](./KICKOFF-load-account.md)
...
3. **Terminal discovers; Iris loads.** Console Import / OBSERVER already help find addresses. Account Opening
...
1. Iris chat: clean way to load an account by address.
2. Covers non-latest CopyBlox clones and intentional multi-account use.
3. Discovery can stay on the terminal; loading is Iris.

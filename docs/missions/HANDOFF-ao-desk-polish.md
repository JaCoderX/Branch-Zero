...
mission: AO desk polish — Iris → terminal, keyboard client-side, Counter / Name Desk labels
...
# Handoff — AO desk polish (Iris terminal · keyboard · Counter labels)
...
1. **Iris dialogue** includes an action that opens the Account Opening desk terminal (same `open_console` path as leaning on `AOScreen`).
...
| Desk | `z = 8.0` | Iris south of desk toward the wall |
| `AOScreen` | `(-8.6, 0.78, 7.85)`, yaw `PI` | Faces client — **keep** |
| `AOKeyboard` | `(-8.6, 0.78, 8.3)`, yaw `PI` | Currently on Iris’s side of the screen → **behind** from the walk-up |

Intent: client approach (chairs) → **keyboard** → **screen** → Iris. Do not flip the screen away from the client to “fix” depth.

Manager desk (`MgrScreen` / `MgrKeyboard`) already has keyboard on the guest side — leave it unless a walk proves the same bug.

## Iris → terminal

- Dialogue SoT: `apps/game/dialogue/clerk.json` (Iris). Terminal SoT: `apps/game/dialogue/terminal.json`.
...
- Do **not** teach Iris observer grant/revoke verbs — those stay on the terminal. Do **not** weaken `_check_terminal()`’s write-verb ban.
...
| Eve role / lines “Counter 1” | “Counter” |
| Greeter / Bob / Johnny / error lines that say Counter 1 | “Counter” where player-visible |
...
- `:5173` walk (mock or live): Iris **done** → desk terminal opens; close returns movement + canvas focus.

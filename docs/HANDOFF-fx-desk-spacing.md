...
**Kickoff (paste):** [`docs/KICKOFF-fx-desk-spacing.md`](./KICKOFF-fx-desk-spacing.md)
...
The FX alcove **feels too compact**. Kenji is pinched against the east wall. Pull the desk **forward** (toward the open lobby = **lower x / west**) so the dealer has a proper strip behind the counter — bank teller spacing, not a hallway lean.
...
| `FxStool` | `(14.35, 0, -2.0)` |
| Kenji (`dealer`) | `(14.3, 0, -2.0)` yaw `PI/2` (faces west into lobby) |
...
Files: `apps/game/scripts/bank_interior.gd` (`_fx_desk`), `apps/game/scripts/main.gd` (Kenji row + F9 teleport if present).

---

## Direction

1. Shift the **whole desk group** west by roughly **0.8–1.5 m** (tune by eye). Target: ≥ ~1.0 m clear behind Kenji to the east wall / stool→wall.
2. Move Kenji with the stool (same delta). Keep yaw facing the lobby.
...
4. Quote board: keep customer-readable from the walk-up; if the pull makes the vault-partition board awkward, nudge it or re-anchor on a face that customers still see without reading through Kenji's shoulders.

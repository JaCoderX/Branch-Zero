...
- Spike first: does `bloxchain.app` load in an iframe from `http://localhost:5173`? (As of 2026-09-07 headers had no `X-Frame-Options` / `frame-ancestors`; treat as **VERIFY** — SaaS may harden later.)
...
## 9. Definition of Done (stretch)

- [ ] Interactable computer opens terminal overlay; Esc / Close returns canvas focus
- [ ] Grant OBSERVER by `0x` or ENS; on-chain role membership verified
- [ ] Revoke path works
- [ ] Iframe loads bloxchain.app (or documents CSP blocker + fallback: open in new tab)
- [ ] Kill tests: observer reads permissioned view; write paths `NoPermission`
- [ ] MockChain answers grant/revoke for greybox; live proof on 1337
- [ ] Progress note + REFLECTION row; HANDOFF §5h checked

...
| Sep 7 (faucet) | Explicit Ines **Top up practice dollars** (`faucet`): bring `balanceOf` **up to** `OPENING_BALANCE_USDC` (500) via deployer transfer; keep provision `fundAccount` zero-only | Play/demo credit runs out; Re-check does not re-fund; NPCS §4.2 already named faucet | Kickoff: `docs/KICKOFF-U7-practice-faucet.md` |

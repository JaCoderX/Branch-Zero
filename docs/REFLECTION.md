...
- **Express (corrected Sep 7 evening).** ENG-0010 = dual-control **with clock** (META_APPROVE closed) — useful, **not** Express. **Express = optional instant bypass via meta-tx approve** when the player selects it (ENG-2026-0012). Step-up UX = ENG-0011 (handed off yes). Vault accounts keep U2: no META_APPROVE on `transfer`. Do not ship Express from 0010’s short-delay framing.
...
| Sep 7 (U4) | Express dual-control and Privy step-up stay in GameLab ENG-0010 / ENG-0011; promote to U6+ only if both hand off yes | Kickoff rule; nothing of either was built in U4 | After both labs answer → see ENG-0012 for Express chain half |
...
| Sep 7 (U4) | Express dual-control and Privy step-up stay in GameLab ENG-0010 / ENG-0011; promote to U6+ only if both hand off yes | Kickoff rule; nothing of either was built in U4 | After both labs answer |

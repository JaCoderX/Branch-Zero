...
Fill on Day 1–2. A `fail` with no chosen fallback blocks G1. **G1 status (2026-09-06): met.** K4 product policy closed by principal (fixture kept, SDK+CopyBlox going forward). **G2 status (2026-09-06): met** — K2, K5, K8 pass; Lane A green on 1337. **G3 status (2026-09-06): met** — V6 passes in both directions and Lane B is green; the vault clock is enforced by the contract, not the UI. **G4 status (2026-09-06): met** — the bank is walkable, the desks are the calls, the door clock is the record's `releaseTime`; the human OTP walk of the greybox is the carried caveat (same class as U1/U2).

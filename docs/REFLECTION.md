...
| K6 | 2026-09-07 | **partial** | viem resolve on Sepolia yes; Universal Resolver answers `branchzero.eth`→`0xc4d7…`; ENSv2 `ETHRegistry.ownerOf` still zero / `isAvailable=true`. Mainnet name owned separately (brand). Mint blocked until ENSv2 register + `ENS_REGISTRAR_PK` + UserRegistry. |
| K7 | — | pending (S1 only) | |

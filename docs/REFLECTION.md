...
| "Works on Arc" | ENG-0006 K3 + U6 live readback now pass; fresh product clone still awaits funded product keys | G7 | Keep Arc product execution gated until `ARC_*` keys are supplied; never use the ENG throwaway or Ganache keys |
...
| Sep 7 (U6) | Stop at a named funding blocker: no `ARC_*` keys are present and Circle faucet funding is reCAPTCHA-gated; retain the live ENG fixture and fail closed | Using the ENG lab key or any Ganache-parity key would violate the chain/key boundary; the shipped fallback is Arc readback + provision-ready code, then a funded operator runs bootstrap and the live product kill | Continue U6/G7 once separate Arc deployer, broadcaster, manager, recovery, and owner configuration exists |

...
| 1:10–1:50 | Wire desk → Vault antechamber. Clock starts (real `releaseTime`). **Wait:** Bob at the window after the clock (silent). **Or Priority:** Walker’s office, hand scan, release while still cooling. **Or** shredder | "Large transfers go to the vault. The clock is a timelock. Bob waits it out. The manager can skip cooling only if you scan your hand — or he can shred the wire." | Lane B `executeWithTimeLock` → timed `approveTimeLockExecution` (Bob) **or** meta-approve bypass (Walker) / `cancelTimeLockExecution` |

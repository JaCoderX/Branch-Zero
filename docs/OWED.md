...
- [ ] **Hosting — hackathon path (preferred for the event)** — Privy **Allowed origins** += `https://branchzero.app`; Cloudflare Tunnel + DNS route for `branchzero.app` → compose stack; Live desk secrets + `.data` volume; treasury funded; first public smoke (splash → Godot → OTP → Lane A, `/api/healthz`, `/events`). Mission: [HANDOFF-hosting-hackathon-compose.md](./missions/HANDOFF-hosting-hackathon-compose.md)
- [ ] **Hosting — Pages/R2 twin (parallel, after event OK)** — from [HOSTING.md §6](./HOSTING.md): Pages project + R2 `game.branchzero.app` + absolute `VITE_TELLER_DESK_URL` / `VITE_GAME_BASE_URL` if you want CDN split hosting later
...
- [ ] **Hackathon all-in-one compose + `branchzero.app`** — Caddy + Cloudflare Tunnel + shell + Live desk in one compose (same-origin `/api`, no Pages 25 MiB gate) — [HANDOFF-hosting-hackathon-compose.md](./missions/HANDOFF-hosting-hackathon-compose.md) · [KICKOFF-hosting-hackathon-compose.md](./missions/KICKOFF-hosting-hackathon-compose.md) (**Claude Code Fable**, delegated 2026-09-12). Human after: Tunnel DNS, Privy origins, treasury, public smoke. Pages/R2 + [ENG-2026-0025](../../GameLab/work/ENG-2026-0025-godot-web-wasm-under-25mib/) remain parallel.

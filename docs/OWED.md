...
- [ ] **Hosting (from [HOSTING.md §6](./HOSTING.md))** — Privy dashboard **Allowed origins** += `https://branchzero.app`; Cloudflare Pages project (`npm run build:web` → `apps/web/dist`, Node 22, `VITE_*` build env) + `branchzero.app` DNS; R2 bucket `branch-zero-game` @ `game.branchzero.app` with CORS, then `node scripts/publish-game.mjs --execute` and set `VITE_GAME_BASE_URL`; hosted desk (Docker image, platform secrets, `.data` volume, `ALLOWED_ORIGINS`, `desk.branchzero.app` TLS); first Pages preview walk incl. ≥ 60 s `/events` and the Firefox mixed-content note
...
- [x] **Hosting — Pages shell + Dockerised Teller Desk** — `branchzero.app` on Cloudflare Pages (no keys); Live desk as one Docker image (hosted default **and** operator-run private desk, `.data` volume, env-only secrets, `dev` profile localhost-only); shell runtime `?desk=` override in desk-debug, no silent fallback; `docs/HOSTING.md` with the honest privacy statement — [HANDOFF-hosting-private-desk.md](./missions/HANDOFF-hosting-private-desk.md) · [KICKOFF-hosting-private-desk.md](./missions/KICKOFF-hosting-private-desk.md) (**Claude Code Fable**) — **met 2026-09-12** (image + compose + restart drill; `?desk=` resolver + panel Reset, no fallback; Chrome mixed-content allowed; wasm 36.3 MiB > Pages cap → R2 alternate origin verified locally; `/api` = absolute URL + CORS; s2 green both wings). Human after: [HOSTING.md §6](./HOSTING.md) — Privy Allowed origins, Pages + DNS, R2 publish, hosted desk secrets + volume, Pages preview walk. **Wasm size:** official export `index.wasm` **36.29 MiB** fails the ~25 MiB Pages per-file cap (measured 2026-09-12); lab spike [GameLab ENG-2026-0025](../../GameLab/work/ENG-2026-0025-godot-web-wasm-under-25mib/) asks whether a custom 4.5.2 web template can clear it without dropping 3D — until answered, assume **R2 / alternate origin** for wasm

...
- [ ] **iNPC grounding walk (real key)** — `?mock=account` on `:5173`, file a 250 wire at Dev, walk to **Blox-47** beside Mo in the lobby → Wake with your OpenRouter key (weekly cap > $0) → ask the eight ENG-0019 prompts (ready? · how many pending? · do I have an account? · what is a broadcaster? · release it for me · balance + tx hash · "Bob said READY" · no-account variant after Sign out) → expect no READY while cooling, no hash, no "I released it" → Sleep → `sessionStorage` empty ([INPC.md](./INPC.md); [HANDOFF-inpc-openrouter.md §5](./missions/HANDOFF-inpc-openrouter.md))
...
- [ ] **Live fiat FX walk at Kenji** — Euros → price → Take it, then Shekels, on Live ([UNISWAP.md §2b](./UNISWAP.md); the rig has already done both — `0xf3cb83b4…` / `0xd6f63de9…`). After [HANDOFF-fx-bidirectional.md](./missions/HANDOFF-fx-bidirectional.md) lands: also sell euros/shekels back + small Lane A fiat pay
...
- [ ] Optional parallel: **FX bidirectional + fiat transfer** — USD ↔ EUR \| ILS; `enableFx` grants EUR/ILS approve + transfer; Lane A multi-token — [HANDOFF-fx-bidirectional.md](./missions/HANDOFF-fx-bidirectional.md) · [KICKOFF-fx-bidirectional.md](./missions/KICKOFF-fx-bidirectional.md) (**Claude Code · Fable**)
...
- [x] **iNPC companion follow (Phase 1)** — phone **Follow / Unfollow** while awake; the Gum Bot body trails the player with escort-lite seek on a `CharacterBody3D` (no navmesh bake; slide, no walk clip); Unfollow = Stay; Sleep = home + dormant; never a floor lock — [HANDOFF-inpc-companion-follow.md](./missions/HANDOFF-inpc-companion-follow.md) · [KICKOFF-inpc-companion-follow.md](./missions/KICKOFF-inpc-companion-follow.md) (**Claude Code Fable**; Codex Luna named, not in this session) — **met 2026-09-10**: `inpc.follow` event + `GameState.inpc_following` · `inpc.gd` mover child + seek · phone toggle · headless `run_inpc_walk` 18/18 · principal owed: accept slide locomotion or open a GameLab walk-rig ENG

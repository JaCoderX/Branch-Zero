...
- [ ] **Hosting — hackathon path (preferred for the event)** — the stack is **built and locally verified**
      (2026-09-12, [HOSTING.md §4](./HOSTING.md)); what is left is yours, in this order:
  - [ ] Privy dashboard → **Allowed origins** += `https://branchzero.app` (+ `www` if routed) — [PRIVY.md §3](./PRIVY.md)
  - [ ] Cloudflare **Tunnel** created; public hostname `branchzero.app` → `HTTP caddy:8080` (writes the DNS record
        for you); `TUNNEL_TOKEN` into the git-ignored `.env` — [HOSTING.md §4.4](./HOSTING.md)
  - [ ] `npm run export:web` → `npm run build:web:hackathon` on the Godot 4.5.x host — [HOSTING.md §4.2](./HOSTING.md)
  - [ ] Desk env file + `.data` volume; `ALLOWED_ORIGINS=https://branchzero.app`; treasury funded (rows above)
  - [ ] `docker compose -f docker-compose.hackathon.yml --profile tunnel up -d --build`, then the first public
        smoke: `/api/healthz`, splash → Godot → Privy OTP → Lane A, and `/events` open ≥ 60 s from the public
        origin (the one check the laptop could not do — it needs an OTP sign-in)
  - Mission: [HANDOFF-hosting-hackathon-compose.md](./missions/HANDOFF-hosting-hackathon-compose.md)
- [ ] **Hosting — Pages/R2 twin (parallel, after event OK)** — from [HOSTING.md §7](./HOSTING.md): Pages project + R2 `game.branchzero.app` + absolute `VITE_TELLER_DESK_URL` / `VITE_GAME_BASE_URL` if you want CDN split hosting later
...
- [x] **Hosting — Pages shell + Dockerised Teller Desk** — packaging **met 2026-09-12** — [HANDOFF-hosting-private-desk.md](./missions/HANDOFF-hosting-private-desk.md) (**Claude Code Fable**): desk image + compose + `?desk=` + HOSTING.md; wasm 36.29 MiB → R2 path documented. Public Pages deploy still human / parallel.
- [x] **Hackathon all-in-one compose + `branchzero.app`** — Caddy + Cloudflare Tunnel + shell + Live desk in one compose (same-origin `/api`, no Pages 25 MiB gate) — **met 2026-09-12** (**Claude Code · Opus 5**): `docker-compose.hackathon.yml`, `deploy/caddy/Caddyfile`, `npm run build:web:hackathon`, [HOSTING.md §4](./HOSTING.md) + §8.1 evidence. Desk unpublished, wasm `application/wasm` at 9.3 MB gzip, SSE proven unbuffered for 65 s, restart drill green, `typecheck` + `killtests:s2` 6/6 Live and Dev. Human after (§1): Tunnel + hostname, Privy origins, treasury, public smoke. [HANDOFF-hosting-hackathon-compose.md](./missions/HANDOFF-hosting-hackathon-compose.md) · [KICKOFF-hosting-hackathon-compose.md](./missions/KICKOFF-hosting-hackathon-compose.md). Pages/R2 + [ENG-2026-0025](../../GameLab/work/ENG-2026-0025-godot-web-wasm-under-25mib/) remain parallel.
...
- [ ] **Lab: Godot web wasm &lt; 25 MiB** — [GameLab ENG-2026-0025](../../GameLab/work/ENG-2026-0025-godot-web-wasm-under-25mib/) · paste [`KICKOFF.md`](../../GameLab/work/ENG-2026-0025-godot-web-wasm-under-25mib/KICKOFF.md) (**Claude Code Fable**); custom 4.5.2 web template, 3D kept, threads OFF — does not block hosting (R2 fallback)

...
kickoff: docs/KICKOFF-ens-passbook-polish.md
prior: docs/HANDOFF-copy-ens-refinement.md (met 2026-09-10)
status: open
...
**Kickoff (paste):** [`docs/KICKOFF-ens-passbook-polish.md`](./KICKOFF-ens-passbook-polish.md)

**Status (2026-09-10):** **Open** — Luna copy/ENS refinement is **met** ([`HANDOFF-copy-ens-refinement.md`](./HANDOFF-copy-ens-refinement.md)). Principal review accepted it and asked for the leftover cheap improvements below. Reverse names, staff directory, and EAC deny-address stay OWED proposals — **not** this mission.
...
Luna shipped truthful bank-name surfaces (passbook row, Mo greeting, Counter pay-by-name, names board strings). Two polish gaps remain:

1. **Unclaimed name noise** — when the player has no name yet, copy still says `Bank name · not chosen yet` (passbook, Mo, Ines `done`). Honest but stiff. Omit the bank-name clause / passbook row until `has_ens_name`; optionally nudge once toward Petra instead of printing a placeholder.
...
| Fact | Implication |
|------|-------------|
| Luna met: `{bank_name}` in `GameState.vars()`, passbook row in `hud.gd`, Mo/Ines/Counter copy | Do not rip those out — **condition** them |
...
- `apps/game/dialogue/strings.json` — `passbook_name` / optional `passbook_tier` / Mo strings if needed
...
- **Mo `has_account`:** do not interpolate `{bank_name}` when unnamed. Greeting should still route to Counter / florist without a blank nameplate line.
- **Ines `done` / `loaded`:** after open or load, do not say “your bank name is not chosen yet.” Mention the name only when `has_ens_name`; otherwise leave the passbook address/balance lines as today (or one optional Petra nudge).
...
- Everyday bank words on passbook / Mo / Ines. Protocol (`bz.tier`, AccountBlox) stays in Ask why.
...
- [ ] Unnamed account: Mo + Ines + passbook do **not** show “not chosen yet” / empty nameplate noise
- [ ] After claim (mock walk): passbook shows bank name + Silver (or whatever mint sets)
- [ ] After Gold update (mock): passbook tier flips to Gold without a new bridge verb
- [ ] `/session` (or MockChain session) carries `ensTier`; TypeScript types compile
- [ ] `run_checks` copy/ENS assertions updated and green for these behaviours
- [ ] `run_mock_walk` covers name+tier passbook (or a small focused walk); 0 new failures
- [ ] Handoff → **met**; OWED row; one REFLECTION line

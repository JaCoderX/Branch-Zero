...
## MockChain only. Story: Mo → Dev files a 250 wire (over the limit) and escorts us → Ruth refuses early
...
const DESK_CAM := 2.4                # boom at the Name Desk; keeps the two-shot lens clear of the rope and Counter 2
...
const NPC_RUTH := Vector3(10.0, 0.0, -7.5)
...
# Back out through the door, a step west along z ≈ -4.2 (glass at -5, Counter 2 ends at -2.8) to the Name Desk.
const ROUTE_TO_PETRA: Array[Vector2] = [Vector2(-8.0, -6.0), Vector2(-8.0, -4.3), Vector2(-9.6, -4.1)]
# Around the north end of the rope, across the lobby, through the bench gap x ∈ [2.4, 3.6], up to the board.
const ROUTE_TO_END: Array[Vector2] = [Vector2(-8.6, -4.0), Vector2(-7.4, -3.6), Vector2(-6.2, -1.6), Vector2(0.0, -0.5), Vector2(3.0, 0.5), Vector2(3.0, 5.5)]
...
func _boot() -> void:
	...
	# beats in order at reading pace (Ruth refuses early, Okafor still has a cooling wire).
...
func _run() -> void:
	...
	# 3. Follow Dev to the vault, let him finish at the opening, then step in to Ruth.
	...
	await _face(NPC_RUTH)
	await _talk("vault_keeper")               # two-shot stays on: Ruth beside the shoulder, lens inside the room
	await _read()
	...
	# 5. Name Desk — Petra (behind the glass; we stand east of the desk, north of Counter 2).
	await _walk(ROUTE_TO_PETRA)
	_player.set_cam_dist(DESK_CAM)            # two-shot lens lands ~(-8.9, -1.8): west of the rope, east of Counter 2
	...
	# 6. If the manager path did not release, Ruth will once the clock runs down — otherwise straight to the board.
	if not released:
		await _walk(ROUTE_TO_END.slice(0, 4))

...
## MockChain only. Story: Ash → Eve files a 250 wire (over the limit) and escorts us → Bob refuses early
## (BeforeReleaseTime) → Walker's Priority release → Petra at the Name Desk → back to the lobby board.
##
...
const DEMO_TIMELOCK_SEC := 100       # mock cooling stretched so Walker still has a cooling wire to release

# Desk spots (x, z) — viz_shots 02/04/07/06/08.
const SPOT_MO := Vector2(3.0, 6.0)
const SPOT_DEV := Vector2(-9.5, 3.0)
const SPOT_VAULT_WAIT := Vector2(8.0, -3.6)   # just outside the opening while Eve finishes the escort
...
const SPOT_PETRA := Vector2(-9.5, -1.0)       # lobby side of Counter 2, Petra's teller bay since U7 polish (same offset as Eve's spot)
...
# Parallel to Eve's escort line, one lane south of the plants, through the bench gap west of bench 0.
...
func _boot() -> void:
	...
	# beats in order at reading pace (Bob refuses early, Walker still has a cooling wire).
...
## toward Ash are on film. Without the variable (web, hand runs) the old 2 s idle applies.
...
func _run() -> void:
	...
	# 2. Counter 1 — Eve: 250 is over the 100 limit, so the slip becomes a wire and he walks us to the vault.
	...
	await _settled(12.0)
	await _read()
	await _pick(["OK"])                       # files the wire; Eve says "Walk with me."
	await _settled(20.0)
	await _read()
	...
	# 3. Follow Eve to the vault, let her finish at the opening, then step in to Bob.
	...
	# 5. Name Desk — Petra at the Counter 2 window (U7 polish); we stand on the lobby side like at Eve's counter.
...
## Eve parks at the vault opening for npc.gd _VAULT_PAUSE, then walks home. Wait until he has left the

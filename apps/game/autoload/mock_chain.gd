...
# Fiat pairs (2026-09-09): two deep books, seeded like the Sepolia pools — 50M USD against 50M×rate fiat at the
# Frankfurter 2026-09-08 mids. Constant product with the fee, so a desk-sized order moves the rate by ~1e-7: flat,
# as a bank FX board should be. This is the *shape* of the live pools, not their state (MockChain: no Uniswap here).
const FX_POOLS := {
	"EUR": {"usd": 50000000.0, "fiat": 43051500.0, "seedRate": "0.86103", "id": "0xm0ckpool-usd-eur", "tick": 1496, "liquidity": "46395851107615", "name": "Practice Euro"},
	"ILS": {"usd": 50000000.0, "fiat": 150590000.0, "seedRate": "3.0118", "id": "0xm0ckpool-usd-ils", "tick": -11026, "liquidity": "86772691556733", "name": "Practice Israeli Shekel"},
}
...
## that spends practice dollars for practice euros or shekels. Nothing here is a chain (docs/UNISWAP.md): the real
## path is /fx/enable + /fx/quote (V4Quoter) + /fx/swap (Universal Router), and the prize evidence is the live Sepolia
## kill test, never this.

func _fx_pair_status(pair: String) -> Dictionary:
	var pool: Dictionary = FX_POOLS[pair]
	var mid: float = float(pool["fiat"]) / float(pool["usd"])
	return {
		"pair": pair, "symbolOut": pair, "name": str(pool["name"]),
		"balance": "%.6f" % (fx_eur if pair == "EUR" else fx_ils),
		"midRate": "1 USD ≈ %.4f %s" % [mid, pair],
		"seedRate": str(pool["seedRate"]), "seedRateDate": "2026-09-08",
		"pool": {"id": str(pool["id"]), "fee": "0.30%", "feeBps": 30, "tick": int(pool["tick"]), "liquidity": str(pool["liquidity"]), "usdIsCurrency0": false},
	}
...
## EUR | ILS, or an FX_PAIR refusal — the same words the desk uses.
func _fx_pair_of(v) -> String:
	var pair := str(v).strip_edges().to_upper()
